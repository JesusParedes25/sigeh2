<?php
	...
	$query = "SELECT * FROM `solicitudes24` ORDER BY `id` DESC LIMIT 1";
	...
	while($imp = $result->fetch_assoc())
	{
		...
		$contenido = $pdf->contenido('Tu producto sera enviado al correo '.$imp['correo'].' y podras revisar el estatus de tu solicitud con el Folio: '.$imp['folio']);
		$pdf->MultiCell(196,5,$contenido,0,'J');

		$pdf->Output('I', 'Solicitud_'.$imp['folio'].'_'.$imp['dependencia'].'.pdf');
